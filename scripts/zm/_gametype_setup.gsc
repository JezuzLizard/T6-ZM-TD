...
struct_class_init_override()
{
	...
	gametype = getDvar( "ztd_gametype" );
	location = getDvar( "ztd_location" );
	print( "Trying to execute the struct location func" );
	if ( array_validate( level.add_struct_gamemode_location_funcs ) )
	{
		print( "struct locations array exists" );
		if ( array_validate( level.add_struct_gamemode_location_funcs[ gametype ] ) )
		{
			print( "struct locations for gametype exist" );
			if ( array_validate( level.add_struct_gamemode_location_funcs[ gametype ][ location ] ) )
			{
				print( "struct locations for gametype and location exist" );
...
register_zombie_spawn( spawner_zone, coordinates, angles, location_script_noteworthy )
{
